...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if (index != _previousPageIndex) {
        if ([_delegate respondsToSelector:@selector(photoBrowser:didDisplayPhotoAtIndex:)]) [_delegate photoBrowser:self didDisplayPhotoAtIndex:index];
...
- (void)_playVideo:(NSURL *)videoURL atPhotoIndex:(NSUInteger)index {
    // Setup player
    _currentVideoPlayerViewController = [[TPPlayVideoCtl alloc] initWithVideoUrl:videoURL];
    __weak MWPhotoBrowser *ws = self;
...
//    } else {
//        AVURLAsset *asset = [AVURLAsset URLAssetWithURL:videoURL options:(self.customHeader ? : @{})];
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    // Force visible
    if (![self numberOfPhotos] || _gridController || _alwaysShowControls) hidden = NO;
...
- (void)setCurrentPhotoIndex:(NSUInteger)index {
    ...
    if (photoCount == 0) {
        ...
        if (index >= photoCount) index = [self numberOfPhotos] - 1;
    }
    _currentPageIndex = index;
    if ([self isViewLoaded]) {
